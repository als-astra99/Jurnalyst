...
import {
  ArrowClockwise, Coins, Plus, TrendUp, TrendDown,
  Briefcase, Warning, CheckCircle,
...
export default function PortfolioPage() {
  ...
  // Fetch semua harga PARALEL dengan Promise.allSettled
  const fetchPrices = useCallback(async () => {
    ...
    const results = await Promise.allSettled(
      holdings
        .filter((h) => h.assets)
        .map((h) =>
          fetch(`/api/price?symbol=${h.assets!.symbol}&type=${h.assets!.asset_type}`)
            .then((r) => r.json())
            ...
            <div className="stitched-card stripe-navy p-6 flex flex-col justify-between h-full">
              ...
              <div className="mt-4">
                ...
                <p className="text-xs mt-1.5" style={{ color: '#94A3B8' }}>
                  {holdings.length} posisi terbuka
                </p>
              </div>
            </div>
          </AnimatedContent>
            ...
            <div className={`stitched-card p-6 flex flex-col justify-between h-full ${totalPnL >= 0 ? 'stripe-green' : 'stripe-red'}`}>
              ...
              <div className="mt-4">
                <p
                  ...
                  style={{ color: totalPnL >= 0 ? '#2F9E6E' : '#D14343' }}
                >
                  {totalPnL >= 0 ? '+Rp' : '-Rp'}
                </p>
                <p
                  className={`font-serif-heading text-2xl md:text-[1.75rem] font-bold font-number-mono leading-none ${totalPnL >= 0 ? 'text-[#2F9E6E]' : 'text-[#D14343]'}`}
                >
                  {Math.abs(totalPnL).toLocaleString('id-ID')}
                </p>
                <p
                  className={`text-xs font-semibold mt-1.5 flex items-center gap-1 ${totalPnL >= 0 ? 'text-[#2F9E6E]' : 'text-[#D14343]'}`}
                >
                  {totalPnL >= 0
          ...
          <div className="stitched-card p-6 rounded-2xl">
            ...
            {loadingData ? (
              ...
              <div className="p-4 rounded-lg bg-amber-50 border border-amber-200 text-amber-900 text-xs flex items-center justify-between">
                ...
                <Link
                  href="/assets"
                  className="px-3 py-1.5 bg-[#B8802E] text-white text-xs font-semibold rounded-md hover:bg-amber-700"
                >
                  Kelola Aset
                </Link>
              </div>
                        ...
                        <div className="sm:text-right">
                          {currentPrice !== undefined ? (
                            <div>
                              ...
                              <p
                                className={`font-serif-heading font-bold text-sm font-number-mono mt-0.5 ${pnl! >= 0 ? 'text-[#2F9E6E]' : 'text-[#D14343]'}`}
                              >
