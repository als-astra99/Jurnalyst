...
function cell(
  text: string,
  opts: { bold?: boolean; width?: number; bg?: string; color?: string } = {}
) {
...
export async function POST(request: NextRequest) {
  ...
  const walletRows: TableRow[] = [
    new TableRow({
      children: ['Nama Dompet', 'Pemasukan', 'Pengeluaran', 'Sisa (Saldo)'].map((t, i) =>
  ...
  const monthRows: TableRow[] = [
    new TableRow({
      children: ['Bulan', 'Pemasukan', 'Pengeluaran', 'Saldo'].map((t, i) =>
  ...
  const yearRows: TableRow[] = [
    new TableRow({
      children: ['Tahun', 'Pemasukan', 'Pengeluaran', 'Saldo'].map((t, i) =>
        cell(t, { bold: true, width: rekapWidths[i], bg: HEADER_BG })
      ),
    }),
  ]
