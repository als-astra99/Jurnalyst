...
import {
  TrendUp, TrendDown, Wallet, Plus, ArrowRight,
  ChartPie, ChartBar, Receipt, BookBookmark, Coins,
} from '@phosphor-icons/react'

// Code-split Recharts: hanya di-load saat dashboard dibuka
const ExpensePieChart = dynamic(
  () => import('./charts').then((m) => ({ default: m.ExpensePieChart })),
  {
    ssr: false,
    loading: () => (
      <div className="h-[300px] flex items-center justify-center">
        <div className="w-24 h-24 rounded-full border-4 border-slate-100 border-t-amber-300 animate-spin" />
      </div>
    ),
  }
)
...
const MonthlyBarChart = dynamic(
  () => import('./charts').then((m) => ({ default: m.MonthlyBarChart })),
  {
    ssr: false,
    loading: () => (
      <div className="h-[300px] flex items-end justify-around gap-2 px-4 pb-8">
        ...
        <AnimatedContent distance={36} duration={0.65} delay={0.19} threshold={0.05}>
          ...
          >
            ...
            <div className="mt-5">
              ...
              <p
                className={`font-serif-heading text-2xl md:text-[1.75rem] font-bold font-number-mono leading-none ${isPositive ? 'text-[#0F1E36]' : 'text-[#922B21]'}`}
              >
                <CountUp to={Math.abs(balance)} duration={2} separator="." />
              </p>
              <p className="text-xs mt-1.5" style={{ color: '#8090A8' }}>
      ...
      <AnimatedContent distance={24} duration={0.65} delay={0.08} threshold={0.05}>
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                'linear-gradient(to right, transparent, rgba(201,151,58,0.5) 40%, rgba(232,180,85,0.5) 60%, transparent)',
            }}
          />
